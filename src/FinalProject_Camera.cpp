...
int main(int argc, const char *argv[])
{
    ...
    cv::VideoWriter video("TTC_outcome.mp4", cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 10, cv::Size(1242, 375), true);
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth)
    {
        ...
        detectObjects((data_buffer.end() - 1)->cameraImg, (data_buffer.end() - 1)->boundingBoxes, confThreshold, nmsThreshold,
        ...
        (data_buffer.end() - 1)->lidarPoints = lidarPoints;
        ...
        clusterLidarWithROI((data_buffer.end() - 1)->boundingBoxes, (data_buffer.end() - 1)->lidarPoints, shrinkFactor, P_rect_xx, R_rect_xx, RT);

        // Visualize 3D objects
        bVis = false;
        if (bVis)
        {
            show3DObjects((data_buffer.end() - 1)->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(2000, 2000), true);
        }
        //bVis = false;
        ...
        cv::Mat imgGray;
        cv::cvtColor((data_buffer.end() - 1)->cameraImg, imgGray, cv::COLOR_BGR2GRAY);
        ...
        descKeypoints((data_buffer.end() - 1)->keypoints, (data_buffer.end() - 1)->cameraImg, descriptors, descriptor_Type);
        ...
        if (data_buffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            matchBoundingBoxes(matches, bbBestMatches, *(data_buffer.end() - 2), *(data_buffer.end() - 1)); // associate bounding boxes between current and previous frame using keypoint matches
            //matchBoundingBoxes1(matches, bbBestMatches, *(dataBuffer.end()-2), *(dataBuffer.end()-1)); // associate bounding boxes between current and previous frame using keypoint matches

            //// EOF STUDENT ASSIGNMENT

            // store matches in current data frame
            (data_buffer.end() - 1)->bbMatches = bbBestMatches;
            ...
            for (auto it1 = (data_buffer.end() - 1)->bbMatches.begin(); it1 != (data_buffer.end() - 1)->bbMatches.end(); ++it1)
            {
                // find bounding boxes associates with current match
                BoundingBox *prevBB, *currBB;
                for (auto it2 = (data_buffer.end() - 1)->boundingBoxes.begin(); it2 != (data_buffer.end() - 1)->boundingBoxes.end(); ++it2)
                {
                ...
                for (auto it2 = (data_buffer.end() - 2)->boundingBoxes.begin(); it2 != (data_buffer.end() - 2)->boundingBoxes.end(); ++it2)
                {
                ...
                if (currBB->lidarPoints.size() > 0 && prevBB->lidarPoints.size() > 0) // only compute TTC if we have Lidar points
                {
                    ...
                    double ttcCamera;
                    clusterKptMatchesWithROI(*currBB, *prevBB, (data_buffer.end() - 2)->keypoints, (data_buffer.end() - 1)->keypoints, (data_buffer.end() - 1)->kptMatches);
                    computeTTCCamera((data_buffer.end() - 2)->keypoints, (data_buffer.end() - 1)->keypoints, currBB->kptMatches, sensorFrameRate, ttcCamera);
                    //// EOF STUDENT ASSIGNMENT

                    bVis = true;
                    if (bVis)
                    {
                        cv::Mat visImg = (data_buffer.end() - 1)->cameraImg.clone();
                        showLidarImgOverlay(visImg, currBB->lidarPoints, P_rect_xx, R_rect_xx, RT, &visImg);
                        cv::rectangle(visImg, cv::Point(currBB->roi.x, currBB->roi.y), cv::Point(currBB->roi.x + currBB->roi.width, currBB->roi.y + currBB->roi.height), cv::Scalar(0, 255, 0), 2);

                        char str[200];
                        char str1[200];
                        sprintf(str, "TTC Lidar : %f s", ttcLidar);
                        sprintf(str1, "TTC Camera : %f s", ttcCamera);

                        putText(visImg, str, cv::Point2f(80, 50), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0, 0, 255));
                        putText(visImg, str1, cv::Point2f(500, 50), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(255, 0, 0));
